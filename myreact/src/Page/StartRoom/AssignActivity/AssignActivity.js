...
export default function AssignActivity() {
  ...
  useEffect(() => {
    const handleQuizResult = (res) => {
      ...
      console.log("🚀 emitting activity_started");
      // 🔥 BROADCAST TO STUDENTS
      socket.emit("activity_started", {
        joinCode,
        activityType: "quiz",
        activitySessionId,
        quizId: res.assignedQuiz.Quiz_ID,
        timerType: quizConfig.timerType,
        timeLimit:
          quizConfig.timerType === "question"
            ? Number(quizConfig.questionTime)
            : quizConfig.timerType === "quiz"
            ? Number(quizConfig.quizTime) * 60
            : null,
      });
